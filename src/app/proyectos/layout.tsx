 ...
 * nueva dentro de `/proyectos` nace protegida sin que nadie se acuerde de
 ...
 */
export default async function LayoutProyectos({ children }: LayoutProps<'/proyectos'>) {
  await exigirSeccion('proyectos', 'ver');
  return (
    <>
      <SoloLectura seccion="proyectos" />
      {children}
    </>
  );
}
